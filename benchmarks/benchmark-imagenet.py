...
TEST_IMAGE_PREFIX = '/Users/yurij/datasets/imagenet-1k/data'

model_instance = model.Model()
datadir = utils.get_app_datadir()
database = db.DB(datadir / 'db.sqlite3')
rclip = RClip(model_instance, database, None)

ids = []
text_vectors = []
for id, text in IMAGENET2012_CLASSES.items():
    ids.append(id)
    text_vectors.append(model_instance.compute_text_features([f'photo of {text}'])[0])
...
for image in tqdm(database.get_image_vectors_by_dir_path(TEST_IMAGE_PREFIX)):
    ...
    similarity = image_features @ text_vectors_np.T
    sorted_similarities = sorted(zip(similarity, range(image_features.shape[0])), key=lambda x: x[0], reverse=True)

    image_class = get_image_class(image['filepath'])

    if image_class == ids[sorted_similarities[0][1]]:
        top1_match += 1
        top5_match += 1
    elif image_class in [ids[sorted_similarities[1][1]], ids[sorted_similarities[2][1]], ids[sorted_similarities[3][1]], ids[sorted_similarities[4][1]]]:
        top5_match += 1
