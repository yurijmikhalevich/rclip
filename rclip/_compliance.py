...
def _legal_files(record: dict[str, Any], policy_dir: Path) -> list[Path]:
  ...
  for declared in record["declared_license_files"]:
    declared_path = dist_info / "licenses" / declared
    fallback_path = dist_info / declared
    if declared_path.is_file():
      result.add(declared_path)
    elif fallback_path.is_file():
      result.add(fallback_path)
    elif record["name"] == "rclip" and (policy_dir.parent / declared).is_file():
      result.add(policy_dir.parent / declared)
    else:
  ...
  if override_dir.is_dir():
    result.update(path for path in override_dir.rglob("*") if path.is_file())
...
def _copy_file(source: Path, destination: Path) -> dict[str, Any]:
  ...
  return {"path": destination.as_posix(), "sha256": digest}
...
def collect_legal_materials(
  ...
  _copy_file(python_license, python_target)
  components.append(
    {
      "name": "cpython",
      "version": python_version,
      "license_files": [python_target.relative_to(output).as_posix()],
    }
  )
...
def _native_candidates(root: Path, excluded_root: Path | None = None) -> list[Path]:
  ...
  for path in root.rglob("*"):
    ...
    if path.suffix.lower() in NATIVE_SUFFIXES or ".so." in lower_name or lower_name.endswith(".appimage"):
...
def _notice_present(legal_dir: Path, filename: str, required_text: str | None = None) -> bool:
  matches = sorted(legal_dir.rglob(filename))
  if not matches:
    return False
...
def verify_bundle(root: Path, legal_dir: Path, policy_path: Path | None, syft_json: Path | None) -> dict[str, Any]:
  ...
  for codec_name, codec_policy in policy.get("codecs", {}).items():
    ...
    if required_notice and not _notice_present(legal_dir, required_notice):
  ...
  for codec in ("av1", "dng", "webp"):
    ...
    if detections[codec]:
      notice = codec_policy["required_notice"]
      required_text = codec_policy.get("required_text")
      if not _notice_present(legal_dir, notice, required_text):
  ...
  if detections["dng"] and policy.get("copyleft", {}).get("libraw", {}).get("must_be_shared", False):
    shared_libraw = any(
      any(marker in path.name.lower() for marker in ("libraw", "raw_r")) for path in _native_candidates(root, legal_dir)
    )
...
def main(argv: list[str] | None = None) -> int:
  ...
  try:
    if args.command == "collect":
      ...
      report = verify_bundle(args.root, args.legal_dir, args.policy, args.syft_json)
      if args.output:
  ...
  except (ComplianceError, OSError, subprocess.CalledProcessError, tomllib.TOMLDecodeError) as error:
